...
trait SyntacticGroupOps extends WithPrinter {
  ...
  implicit class XtensionSyntacticGroup(val leftGroup: SyntacticGroup) {
    ...
    private def wrap(`(`: Option[Doc], tree: Tree, `)`: Option[Doc], side: Side): Doc = {
    ...
    private def wrap0(tree: Tree, `(`: Option[Doc], doc: Doc, `)`: Option[Doc], side: Side): Doc = {
    ...
    private def wrap1(
      ...
      if (TreeOps.groupNeedsParenthesis(leftGroup, rightGroup, side)) {
        ...
        `(`.getOrElse(empty) + doc + `)`.getOrElse(empty)
      }
    }
  }
}