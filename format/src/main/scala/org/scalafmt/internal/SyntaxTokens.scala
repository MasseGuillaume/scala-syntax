...
object SyntaxTokens {
  ...
  implicit class XtensionDefnClassSyntax(val tree: Defn.Class) extends AnyVal {
    ...
    def tokensLeftBracket: Option[LeftBracket] = 
      tree.tparams.headOption.map(tparam =>
        tree.findBetween[LeftBracket](_.name, _ => tparam).get
      )
    def tokensRightBracket: Option[RightBracket] =
      tree.tparams.lastOption.map(tparam =>
        tree.ctor.paramss match {
          ...
          case xs :: _ =>
            xs match {
              ...
              // class A[T](b: B)
              case x :: _ => tree.findBetween[RightBracket](_ => tparam, _ => x).get
            }
        }
      )
